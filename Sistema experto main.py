...
def adivinar():
    ...
    while questionIndex < len(questions):
        ...
        if response:
            ...
            # Si la pregunta es del tipo A, aumentar el contador de respuestas afirmativas
            if tipo_pregunta_actual == "A.":
                respuestas_afirmativas_a += 1
            # Si la pregunta es G.2, guardar la respuesta
            if tipo_pregunta_actual == "G.1" and numero_pregunta_actual == 1:
                respuesta_g2 = "Si"
        else:
            answers[shuffledIndexes[questionIndex]] = "No"
            userSelections += f"{tipo_pregunta_actual}.{numero_pregunta_actual}No"
            # Si la pregunta es G.2, guardar la respuesta
            if tipo_pregunta_actual == "G.2" and numero_pregunta_actual == 1:
                respuesta_g2 = "No"

        guardar_respuesta(f"Pregunta {tipo_pregunta_actual}.{numero_pregunta_actual}: " +
                          pregunta_actual + ": " +
        ...
        # Si se han respondido tres preguntas afirmativas del tipo A, pasar al tipo B
        if tipo_pregunta_actual == "A." and respuestas_afirmativas_a == 1:
            questionIndex = obtener_tipo_pregunta_index("B.")
        else:
            questionIndex += 1

        # Si la pregunta actual es del tipo B, y no tiene la misma respuesta que G.2, saltarla
        if tipo_pregunta_actual == "B." and questionIndex < len(shuffledIndexes) and answers[shuffledIndexes[questionIndex]] != respuesta_g2:
            questionIndex += 1
...
def should_predict():
    ...
    matching_count = sum(1 for answer in answers if answer == "Si" or answer == "No")
...
def display_prediction():
    ...
    try:
        with open(nombre_archivo, 'r') as reader:
            max_matching_percentage = 60.0
            predicted_pokemon = ""
            user_responses_counter = Counter(userSelections)
            for line in reader:
                parts = line.split(":")
                if len(parts) > 1:
                    pokemon = parts[0]
